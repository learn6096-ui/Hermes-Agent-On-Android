...
# non-critical commands that may fail (update-alternatives, npm, etc).
# We use explicit error checks on critical steps instead.
...
# Try Python 3.13, fall back to system python3
PYTHON_CMD="python3"
if apt-get install -y python3.13 python3.13-venv python3.13-dev 2>&1; then
    ...
    apt-get install -y python3 python3-pip python3-venv python3-dev 2>&1 || {
        echo "❌ Failed to install Python. Cannot proceed."
        exit 1
    }
fi
...
if ! $PYTHON_CMD --version 2>&1; then
    echo "❌ Python '${PYTHON_CMD}' not working."
    exit 1
fi

# Clone hermes-agent (with retry)
...
if [ ! -d "$HOME/hermes-agent/.git" ]; then
    ...
    if [ "$CLONE_OK" = false ]; then
        echo "❌ Failed to clone repository. Check internet."
        exit 1
    fi
else
